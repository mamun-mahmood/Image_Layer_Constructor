...
const addVarriation = (id) => {
  ...
  <h5 style="text-align: center;">Varriation ${btnCount -1}</h5>
    <div class="display_flex_style">
      ...
      <div><input id="layer_img" type="file" onchange="addLayer(2)" /></div>
    </div>
  </div>
...
const addLayer = (id) => {
  layerCount++;
  const mainDiv = document.getElementById(`layer_container`);
  const subDiv = document.createElement("div");
  ...
  <div><input id="layer_img" onchange="addLayer()" type="file" /></div>
</div>`;
  subDiv.innerHTML = newDiv;
  mainDiv.appendChild(subDiv);
};
